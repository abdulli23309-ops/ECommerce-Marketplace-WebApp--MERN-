    ...
    <div style={{ backgroundColor: "var(--bg-secondary)", minHeight: "100vh", padding: "2rem" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "var(--text-primary)", margin: 0 }}>Shipments</h1>
          <p style={{ color: "var(--text-secondary)", marginTop: "0.25rem" }}>Track and manage order shipments</p>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)", backgroundColor: "var(--bg-secondary)" }}>
                  ...
                  <th style={{ padding: "0.75rem 1.25rem", textAlign: "left", fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase" }}>Carrier</th>
                  <th style={{ padding: "0.75rem 1.25rem", textAlign: "left", fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase" }}>Tracking</th>
                  ...
                  <th style={{ padding: "0.75rem 1.25rem", textAlign: "left", fontSize: "0.75rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase" }}>Created</th>
                </tr>
              </thead>
              <tbody>
                {shipments.map((s) => (
                  ...
                  >
                    <td style={{ padding: "0.75rem 1.25rem", fontWeight: 500, color: "var(--text-primary)" }}>{s.id?.toString().slice(0, 8).toUpperCase()}</td>
                    ...
                    <td style={{ padding: "0.75rem 1.25rem", color: "var(--text-secondary)" }}>{s.carrier || "N/A"}</td>
                    <td style={{ padding: "0.75rem 1.25rem", color: "var(--text-secondary)" }}>{s.trackingNumber || "N/A"}</td>
                    <td style={{ padding: "0.75rem 1.25rem" }}>
                      <span style={getStatusBadgeStyle(s.status)}>{s.status}</span>
                    </td>
                    <td style={{ padding: "0.75rem 1.25rem", color: "var(--text-secondary)" }}>{s.createdAt ? new Date(s.createdAt).toLocaleDateString() : "N/A"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {modalOpen && selectedShipment && (
          ...
          >
            ...
            >
              ...
              <p><strong>Status:</strong> <span style={getStatusBadgeStyle(selectedShipment.status)}>{selectedShipment.status}</span></p>
                ...
                <div style={{ marginTop: "1rem" }}>
                  ...
                  {selectedShipment.trackingHistory.map((th, i) => (
                    <div key={i} style={{ fontSize: "0.9rem", marginLeft: "1rem" }}>
                      <span>{th.status}</span> {th.note && <span>– {th.note}</span>}{" "}
                      <span style={{ color: "var(--text-secondary)" }}>{new Date(th.timestamp).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              )}
